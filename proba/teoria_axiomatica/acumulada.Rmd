...
sol6 <- mean(datos>=xdic)  # P(X ≥ xdic)
sol7 <- mean((datos>=primero)&(datos<tercero))  # P(primero ≤ X < tercero)
sol8 <- 0  # lim x→-∞
sol9 <- 1  # lim x→+∞
...
$$P(`r primero` \leq X< `r tercero`)=F_X(`r tercero`^-)-F_X(`r primero`^-)=`r sol7`.$$
...
exsolution: `r fmt(sol1,4)`|`r fmt(sol2,4)`|`r fmt(sol5,4)`|`r fmt(sol4,4)`|`r fmt(sol6,4)`|`r fmt(sol7,4)`|`r fmt(sol8,4)`|`r fmt(sol9,4)`
exname: acumulada
